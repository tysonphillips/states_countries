...
class AdminStates extends StatesCountriesController {
	...
	public function add() {
		...
		// Add the state
		if (!empty($this->post)) {
			$data = array_merge($this->post, array('country_alpha2' => $country->alpha2));
			...
			if (($errors = $this->StatesCountriesStates->errors())) {
				...
				$this->flashMessage("message", Language::_("AdminStates.!success.added", true, $this->Html->_($state->name, true)), null, false);
	...
	public function edit() {
		...
		if (!empty($this->post)) {
			...
			if (($errors = $this->StatesCountriesStates->errors())) {
				...
				$this->redirect($this->base_uri . "plugin/states_countries/admin_states/edit/" . $country->alpha2 . "/" . $new_state->code);
			}
		}
	...
	public function delete() {
		...
		if ($this->StatesCountriesStates->errors()) {
			...
			$this->flashMessage("message", Language::_("AdminStates.!success.deleted", true, $this->Html->_($state->name, true)), null, false);
		}
