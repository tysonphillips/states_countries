...
class AdminCountries extends StatesCountriesController {
	...
	public function preAction() {
		...
		$this->uses(array("StatesCountries.StatesCountriesCountries"));
	...
	public function add() {
		...
		if (!empty($this->post)) {
			...
			if (($errors = $this->StatesCountriesCountries->errors())) {
				...
				$this->flashMessage("message", Language::_("AdminCountries.!success.added", true, $this->Html->_($country->name, true)), null, false);
	...
	public function edit() {
		...
		if (!empty($this->post)) {
			...
			if (($errors = $this->StatesCountriesCountries->errors())) {
				...
				$this->redirect($this->base_uri . "plugin/states_countries/admin_countries/edit/" . $new_country->alpha2);
			}
		}
	...
	public function delete() {
		// Require a country be given
		if (!isset($this->post['alpha2']) || !($country = $this->StatesCountriesCountries->get($this->post['alpha2']))) {
			$this->redirect($this->base_uri . "plugin/states_countries/admin_main/");
		}
		...
		if ($this->StatesCountriesCountries->errors()) {
			...
			$this->flashMessage("message", Language::_("AdminCountries.!success.deleted", true, $this->Html->_($country->name, true)), null, false);
		}
