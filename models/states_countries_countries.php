    ...
    private function updateCountryReferences($from_alpha2, $to_alpha2) {
        foreach ($this->table_references as $table) {
            $this->Record->where("country", "=", $from_alpha2)
                ->update($table, array('country' => $to_alpha2));
        }
        
        $this->Record->where("country_alpha2", "=", $from_alpha2)
            ->update("states", array('country_alpha2' => $to_alpha2));
    }
		...
		$rules = array(
			'alpha2' => array(
				...
				'in_use' => array(
					'rule' => (bool)$country,
					'negate' => true,
					'message' => $this->_("StatesCountriesCountries.!error.alpha2.in_use", $this->ifSet($country->alpha2), $this->ifSet($country->name))
				)
			),
			'alpha3' => array(
				'format' => array(
